---
title: Arrays
url: https://surrealdb.com/docs/surrealql/datamodel/arrays
crawled_at: 2026-03-25 18:40:19
---
...
```
-- Return a full arrayRETURN [1,2,3,4,5];-- Return the first ("zeroeth") itemRETURN [1,2,3,4,5][0];-- Return indexes 0 up to and including 2 of an arrayRETURN [1,2,3,4,5][0..=2];
```

Output

```
-------- Query 2 (200µs) --------[	1,	2,	3,	4,	5]-------- Query 3 (99.999µs) --------1-------- Query 4 (100.001µs) --------[	1,	2,	3]
```
...
```
-- Even this returns an arraySELECT * FROM 9;-- Use the `ONLY` clause to return a single itemSELECT * FROM ONLY 9;-- Error: array has more than one itemSELECT * FROM ONLY [1,9];
```

Output

```
-------- Query 1  --------[	9]-------- Query 2 --------9-------- Query 3 --------'Expected a single result output when using the ONLY keyword'
```
...
```
[	{		first_mayor: 'Papa Smurf',		name: 'Smurfville',		population: 55	}]
```
...
```
[	'At index 0 we got a 1!',	'At index 1 we got a 2!',	'At index 2 we got a 3!']
```
...
```
[	{		coordinates: (98, 65.7),		item: 0,		name: 'Some city'	},	{		coordinates: (0, 0.1),		item: 1,		name: 'Other city'	}]
```
