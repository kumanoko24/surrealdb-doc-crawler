---
title: None and null
url: https://surrealdb.com/docs/surrealql/datamodel/none-and-null
crawled_at: 2026-03-25 18:40:32
---
...
```
CREATE person:two;CREATE person:one SET children = [person:two];UPDATE person:one SET children = NONE;SELECT * FROM person;
```
...
```
[  {     children: NULL,     id: person:dgwjn0ldg8ep3e8y39jw  }]
```
...
```
DEFINE FUNCTION fn::do_stuff() -> NONE {  // Code that should return nothing};DEFINE FIELD middle_name ON TABLE user TYPE string | NONE; // Equivalent to option<string>DEFINE FIELD value ON temperature TYPE float | decimal | NONE; // Equivalent to option<float|decimal>
```
