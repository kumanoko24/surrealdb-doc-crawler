...
crawled_at: 2026-03-25 18:42:46
---
...
```
-- Enable scope authentication directly in SurrealDBDEFINE SCOPE account SESSION 24h	SIGNUP ( CREATE user SET email = $email, pass = crypto::argon2::generate($pass) )	SIGNIN ( SELECT * FROM user WHERE email = $email AND crypto::argon2::compare(pass, $pass) );
```
...
```
-- Create a SCOPE if it does not already existDEFINE SCOPE IF NOT EXISTS example;
```
