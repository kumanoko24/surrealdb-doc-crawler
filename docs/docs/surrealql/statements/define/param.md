---
title: DEFINE PARAM statement
url: https://surrealdb.com/docs/surrealql/statements/define/param
crawled_at: 2026-03-25 18:42:45
---
...
```
DEFINE PARAM [ OVERWRITE | IF NOT EXISTS ] $@name     VALUE @value    [ COMMENT @string ]    [ PERMISSIONS [ NONE | FULL | WHERE @condition ] ]
```
DEFINEPARAMOVERWRITEIFNOTEXISTS$@nameVALUE@valueCOMMENT@stringPERMISSIONSNONEFULLWHERE@condition
## Example usage
...
```
-- Create a PARAM if it does not already existDEFINE PARAM IF NOT EXISTS $example VALUE 123;
```
...
```
-- Create an PARAM and overwrite if it already existsDEFINE PARAM OVERWRITE $example VALUE 123;
```
