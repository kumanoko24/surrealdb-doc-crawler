---
title: DEFINE CONFIG statement
url: https://surrealdb.com/docs/surrealql/statements/define/config
crawled_at: 2026-03-25 18:42:31
---
...
```
DEFINE CONFIG [ OVERWRITE | IF NOT EXISTS ]  ( API [ MIDDLEWARE @expression, .. ] [ PERMISSIONS [ NONE | FULL | @expression ] ]  | GRAPHQL       [ AUTO | NONE ]      [ TABLES (AUTO | NONE | INCLUDE table1, table2, ...) ]      [ FUNCTIONS (AUTO | NONE | INCLUDE [function1, function2, ...] | EXCLUDE [function1, function2, ...]) ]  )
```
DEFINECONFIGOVERWRITEIFNOTEXISTSAPIMIDDLEWARE@expression, ..PERMISSIONSNONEFULL@expressionGRAPHQLAUTONONETABLESAUTONONEINCLUDEtable1, table2, ...FUNCTIONSAUTONONEINCLUDE[function1, function2, ...]EXCLUDE[function1, function2, ...]
## DEFINE CONFIG API
...
```
DEFINE CONFIG API    MIDDLEWARE         api::res::headers({            'Access-Control-Allow-Origin': '*'        });DEFINE API "/test" FOR get THEN {};DEFINE API "/test2" FOR get THEN {    {        body: {            some: "data"        }    }};api::invoke("/test");api::invoke("/test2");
```
...
```
-------- Query --------{  body: NONE,  context: {},	headers: {		"access-control-allow-origin": '*'	},	status: 200}-------- Query --------{	body: {		some: 'data'	},  context: {},	headers: {		"access-control-allow-origin": '*'	},	status: 200}
```
...
```
DEFINE CONFIG API    MIDDLEWARE         api::timeout(10s),        api::res::headers({            'Access-Control-Allow-Origin': '*'        });DEFINE API OVERWRITE "/test"    FOR get         MIDDLEWARE            api::timeout(1µs)        THEN {            RETURN {                status: 200,                body: {                     data: (SELECT * FROM person),                    however: "This will probably never return because the timeout is 1 microsecond"                }            };        };api::invoke("/test");
```
...
```
-- Define GraphQL configurationDEFINE CONFIG GRAPHQL  AUTO;
```
...
```
-- Define GraphQL configuration only if it does not already existDEFINE CONFIG GRAPHQL IF NOT EXISTS TABLES AUTO FUNCTIONS AUTO;
```
...
```
-- Redefine GraphQL configuration, overwriting existing configurationDEFINE CONFIG OVERWRITE GRAPHQL TABLES INCLUDE user, post FUNCTIONS NONE;
```
...
```
-- Define GraphQL configurationDEFINE CONFIG GRAPHQL TABLES AUTO FUNCTIONS AUTO;
```
...
```
-- Define GraphQL configuration to include all tables automaticallyDEFINE CONFIG GRAPHQL TABLES AUTO;-- Define a table and insert dataDEFINE TABLE foo SCHEMAFUL;DEFINE FIELD val ON foo TYPE int;CREATE foo:1 SET val = 42;CREATE foo:2 SET val = 43;
```
...
```
-- Define GraphQL configuration to include only specific tablesDEFINE CONFIG OVERWRITE GRAPHQL TABLES INCLUDE foo;
```
