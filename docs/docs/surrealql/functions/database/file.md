...
crawled_at: 2026-03-25 18:43:05
---
...
```
f"my_bucket:/my_book.txt".get();<string>f"my_bucket:/my_book.txt".get();
```
...
```
file::list("my_bucket", { limit: 1 });file::list("my_bucket", { limit: 0 });
```
...
```
file::list("my_bucket", { prefix: "some" });file::list("my_bucket", { prefix: "someBOOOEOEOK" });
```
...
```
file::list("my_bucket", { start: "a" });file::list("my_bucket", { start: "m" });
```
...
```
DEFINE BUCKET my_bucket BACKEND "memory";-- Creates file and adds dataf"my_bucket:/my_book.txt".put_if_not_exists("Once there were four children whose names were Peter, Susan...");-- Does nothingf"my_bucket:/my_book.txt".put_if_not_exists("Or were there? I don't quite remember.");<string>f"my_bucket:/my_book.txt".get();
```
