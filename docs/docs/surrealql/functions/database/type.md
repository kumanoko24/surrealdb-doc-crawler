...
crawled_at: 2026-03-25 18:43:32
---
...
```
RETURN type::bytes("A few bytes");-- b"4120666577206279746573"
```
...
```
RETURN type::decimal("12345");-- 12345dec
```
...
```
CREATE person:test SET title = 'Mr', name.first = 'Tobie', name.last = 'Morgan Hitchcock';LET $param = 'name.first';SELECT type::field($param), type::field('name.last') FROM person;SELECT VALUE { 'firstname': type::field($param), lastname: type::field('name.last') } FROM person;SELECT VALUE [type::field($param), type::field('name.last')] FROM person;
```

Output

```
[	{		id: person:test,		title: 'Mr',		name: {			first: 'Tobie',			last: 'Morgan Hitchcock',	    }	}]
```
...
```
LET $omit = "id";CREATE person SET name = "Galen", surname = "Pathwarden", age = 19;SELECT * OMIT type::field($omit) FROM person;
```

Output

```
[	{		age: 19,		name: 'Galen',		surname: 'Pathwarden'	}]
```
...
```
CREATE person:test SET title = 'Mr', name.first = 'Tobie', name.last = 'Morgan Hitchcock';LET $param = ['name.first', 'name.last'];SELECT type::fields($param), type::fields(['title']) FROM person;SELECT VALUE { 'names': type::fields($param) } FROM person;SELECT VALUE type::fields($param) FROM person;
```
...
```
LET $omit = ["id", "age"];CREATE person SET name = "Galen", surname = "Pathwarden", age = 19;SELECT * OMIT type::fields($omit) FROM person;
```

Output

```
[	{		name: 'Galen',		surname: 'Pathwarden'	}]
```
...
```
type::file("my_bucket", "file_name") == f"my_bucket:/file_name";-- true
```

Once a bucket has been defined, operations using one of the file functions can be performed on the file pointer.

```
DEFINE BUCKET my_bucket BACKEND "memory";type::file("my_bucket", "file_name").put("Some data inside");type::file("my_bucket", "file_name").get();
```
...
```
RETURN type::float("12345");-- 12345f
```
...
```
type::of(2022dec);        -- 'decimal';type::of(["some", 9]);    -- 'array';type::of((50.0, 9.9));    -- 'geometry<point>'
```
...
```
RETURN type::range([1, 2]);-- 1..2RETURN type::range(1..10);-- 1..10RETURN type::range([1,9,4]);-- 'Expected a range but cannot convert [1, 9, 4] into a range'
```
...
```
LET $tb = "person";LET $id = "tobie";RETURN type::record($tb, $id);
```

An example of this function being used to turn an array of objects into records to be created or upserted:

```
FOR $data IN [	{		id: 9,		name: 'Billy'	},	{		id: 10,		name: 'Bobby'	}] {	UPSERT type::record('person', $data.id) CONTENT $data;};
```
...
```
FOR $data IN [	{		name: 'Billy',		num: 9	},    {		name: 'Bobby',		num: 10	},].map(|$o| {    id: $o.num,    name: $o.name}) {    UPSERT type::record('person', $data.id) CONTENT $data;};
```
...
```
type::record("person", person:mat);-- person:mat
```
...
```
-- Both return person:tobietype::record(person:tobie);type::record('person:tobie');
```

The optional second argument allows an assertation that the record passed in is of this table name.

```
type::record('person:tobie', 'person'); -- person:tobietype::record('person:tobie', 'cat'); -- "Expected a record<cat> but cannot convert 'person:tobie' into a record<cat>"
```
...
```
LET $record = rand::enum(person:tobie, cat:tobie);type::record($record, 'person');
```
...
```
-- Contains some invalid bytestype::string_lossy(<bytes>[83, 117, 114, 255, 114, 101, 97, 254, 108, 68, 66]);-- valid bytestype::string_lossy(<bytes>[ 83, 117, 114, 114, 101, 97, 108, 68, 66 ]);
```

Output

```
-------- Query --------'Sur�rea�lDB'-------- Query --------'SurrealDB'
```
...
```
RETURN type::is_array([ 'a', 'b', 'c' ]);-- true
```
...
```
RETURN type::is_bytes("I am not bytes");-- false
```
...
```
RETURN type::is_collection("I am not a collection");-- false
```
...
```
RETURN type::is_datetime(time::now());-- true
```
...
```
RETURN type::is_decimal(<decimal> 13.5719384719384719385639856394139476937756394756);-- true
```
...
```
RETURN type::is_duration('1970-01-01T00:00:00');-- false
```
...
```
RETURN type::is_float(<float> 41.5);-- true
```
...
```
RETURN type::is_geometry((-0.118092, 51.509865));-- true
```
...
```
RETURN type::is_int(<int> 123);-- true
```
...
```
RETURN type::is_line("I am not a line");-- false
```
...
```
RETURN type::is_multiline("I am not a multiline");-- false
```
...
```
RETURN type::is_multipoint("I am not a multipoint");-- false
```
...
```
RETURN type::is_multipolygon("I am not a multipolygon");-- false
```
...
```
RETURN type::is_number(123);-- true
```
...
```
RETURN type::is_object({ hello: 'world' });-- true
```
...
```
RETURN type::is_point((-0.118092, 51.509865));-- true
```
...
```
RETURN type::is_polygon("I am not a polygon");-- false
```
...
```
type::is_range(0..1);-- true// method syntax(0..1).is_range();-- true
```
...
```
RETURN type::is_record(user:tobie);-- true
```
...
```
RETURN type::is_record(user:tobie, 'test');-- false
```
...
```
RETURN type::is_string("abc");-- true
```
...
```
RETURN type::is_uuid(u"018a6680-bef9-701b-9025-e1754f296a0f");-- true
```
...
```
-- Traditional syntaxtype::is_record(r"person:aeon", "cat");-- Method chaining syntaxr"person:aeon".is_record("cat");
```
