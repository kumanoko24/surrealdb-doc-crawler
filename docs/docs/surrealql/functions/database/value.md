...
crawled_at: 2026-03-25 18:43:34
---
...
```
'SurrealDB'.chain(|$n| $n + ' 3.0');-- 'SurrealDB 3.0''SurrealDB'.chain(|$n| "Something else");
```
...
```
{ company: 'SurrealDB', latest_version: '2.0' }    .chain(|$name| <string>$name)    .replace('SurrealDB', 'SURREALDB!!!!!');
```
...
```
[	{		op: 'change',		path: '/',		value: '@@ -1,5 +1,6 @@ tobi-e+as'	}]
```
...
```
[	{		op: 'change',		path: '/company',		value: '@@ -2,8 +2,10 @@ urrealDB+!!'	},	{		op: 'add',		path: '/latest_version',		value: '2.0'	},	{		op: 'add',		path: '/location',		value: city:london	}]
```
...
```
LET $company = {    company: 'SurrealDB',    latest_version: '1.5.4'};$company.patch([{		'op': 'replace',		'path': 'latest_version',		'value': '3.0'}]);
```

Response

```
{	company: 'SurrealDB',	version: '3.0'}
```
