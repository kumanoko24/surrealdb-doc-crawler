...
crawled_at: 2026-03-25 18:42:03
---
...
```
string::split("SurrealDB 2.0 is on its way!", " ");array::len([1,2,3]);type::is_number(10);type::record("cat", "mr_meow");
```
...
```
-- Old syntaxtype::is::record(person:one);-- Method syntaxperson:one.is_record();-- New syntax now matches method syntaxtype::is_record(person:one);
```
