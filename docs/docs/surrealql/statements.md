...
crawled_at: 2026-03-25 18:40:56
---
...
```
SELECT * FROM person;DELETE person;REMOVE TABLE person;
```

Output before 3.0

```
-------- Query 1 --------[]-------- Query 2 --------[]-------- Query 3 --------"The table 'person' does not exist"
```
...
```
SELECT * FROM person;DELETE person;
```

Output

```
-------- Query --------"The table 'person' does not exist"-------- Query --------"The table 'person' does not exist"
```

However, statements that create a resource will not return an error unless the database is defined as `STRICT`. Instead, they will automatically define the needed table (and even use the $session parameter to define the database and namespace if necessary) so that the query will work.
...
```
SELECT * FROM person; -- ErrorDELETE person;        -- ErrorCREATE person:one;    -- SucceedsDELETE person;        -- Empty arrayREMOVE TABLE person;  -- NONE
```
